...
const Navbar = () => {
  ...
  return (
    <>
    <nav className={`md:flex justify-center items-center gap-x-72 font-bold hidden w-full z-10 shadow-sm shadow-slate-300 bg-white fixed top-[-40px] right-0 py-5 rounded-md md:mt-10 p-10 `}>
      <Link to='/'>
      ...
      <ul className='md:flex justify-between items-center gap-x-10 hidden'>
         ...
         <Link to='/cart' className='relative bottom-0 hover:text-prim hover:font-bold'>
          {count > 0 && <span className='absolute bottom-5 left-3 text-prim font-bold'>{count}</span>}
          <BiSolidCartAlt size={30}/>
        </Link>
        <Link className='py-3 px-8 transition duration-300 ease-in-out hover:text-white hover:bg-slate-600  bg-black text-white rounded-full' to='/login'>Login</Link>
